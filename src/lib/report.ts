...
export function generateMarkdownReport(input: {
  ...
  const targetPeriodRequestLine = input.goal?.target_date
    ? "- 目標体重と目標達成日から設定されている期間補正が、減量・増量・リコンプの目的に対して現実的かを評価してください。急すぎる/遅すぎる場合は、目標達成日の変更案またはkcal補正案を出してください。"
  ...
  const targetBodyCompositionText = typeof input.goal?.target_fat_mass_kg === "number" && typeof input.goal?.target_lean_mass_kg === "number"
    ? `脂肪 ${input.goal.target_fat_mass_kg}kg${typeof input.goal.target_fat_mass_change_kg === "number" ? ` (${formatDiff(input.goal.target_fat_mass_change_kg, "kg")})` : ""} / 除脂肪 ${input.goal.target_lean_mass_kg}kg${typeof input.goal.target_lean_mass_change_kg === "number" ? ` (${formatDiff(input.goal.target_lean_mass_change_kg, "kg")})` : ""}`
    : "未設定";
...
- 期間補正: ${typeof input.goal?.target_daily_calorie_adjustment === "number" ? `${formatDiff(input.goal.target_daily_calorie_adjustment, "kcal/日")}` : "未設定"}
- 体重変化ペース: ${typeof input.goal?.target_weight_change_per_week_kg === "number" ? `${formatDiff(input.goal.target_weight_change_per_week_kg, "kg/週")}` : "未設定"}
- 体組成目標: ${targetBodyCompositionText}
