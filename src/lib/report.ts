...
export function generateMarkdownReport(input: {
  ...
  const cheatDayRequestLine = cheatDayDates.length
    ...
    : "";
  const specialModeRequestLine = specialModeDays.length
    ? "- 対象範囲に特別モードの日があります。旅行・イベント期間として通常の目標判定から一部例外扱いにし、外食、塩分、炭水化物、移動疲労、水分変動を考慮してください。記録継続、たんぱく質確保、胃腸負担、通常復帰の提案を優先してください。"
    : "";
...
- 特別モード: ${specialModeDays.length ? specialModeDays.map((day) => `${day.date} ${day.label}${day.isTest ? "（テスト）" : ""}`).join(", ") : "なし"}
...
- 特別モードがある場合は、旅行・イベント中であることを明記し、この期間を通常日とは分けて評価してください。
...
- 特別モード: ${specialModeDays.length ? `${specialModeDays.length}日 (${specialModeDays.map((day) => day.label).join(", ")})` : "なし"}
