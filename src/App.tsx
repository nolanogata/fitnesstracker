        ...
        {step === "prompt" && (
          <div className="mt-4 space-y-3">
            <div className="rounded-3xl border border-line bg-rice/60 p-3 text-xs font-semibold leading-relaxed text-moss">
              <p>1. 下のプロンプトをコピーしてChatGPTに貼り付けます。</p>
              <p className="mt-1">2. 写真・バーコード・栄養成分表示も一緒に添付して送信します。</p>
              <p className="mt-1">3. ChatGPTのコードブロックをコピーして、この画面の次ステップに貼り付けます。</p>
            </div>
...
- 返答は説明文なし、\`\`\`json から始まるコードブロック1つだけにする。ChatGPTのコピーボタンでそのままコピーできる形にする
...
function parseAiFoodBridgeText(text: string): { items: AiFoodBridgeItem[]; error?: string } {
  ...
  if (!candidates.length) {
    return { items: [], error: "JSONが見つかりません。ChatGPTのコードブロックをコピーして貼り付けてください。" };
  }
  ...
  return { items: [], error: "JSONとして読み取れませんでした。ChatGPTのコードブロック全体をコピーして貼り付けてください。" };
}
